...
fetch("projects.json")
  ...
  .then((data) => {
    ...
    function changeDescription() {
      const projectDescriptionDiv = document.querySelector(
        "#project-description"
      );
      const selectedSlide = projectsList.querySelector(
...
function generateProject(project) {
  const { id, name, category, thumbnail, images, description, collaborator } =
    project;
  ...
  projectBox.innerHTML = `
            ...
            ${images
              .map(
                (i) =>
                  `<a data-fancybox="${id}" href="${i}" class="zoom-btn project-image invisible"></a>`
              )
              .join("")}
            </a>
        `;
      ...
      (function () {
        ...
        $(".filter-buttons")
          ...
          .on("click", function () {
            ...
            return (
              $(".filter-buttons").find("button").removeClass("active"),
...
function generateRoom(room) {
  ...
  roomBox.innerHTML = `
            ...
            ${images
              .map(
                (i) =>
                  `<a data-fancybox="${id}" href="${i}" class="zoom-btn rooms-image invisible"></a>`
              )
              .join("")}
        </a>
    `;
...
jQuery(document).ready(function ($) {
  ...
  function adjustSlideDimensions() {
    ...
    $(".carousel .slider ul li").css({
      width: sliderWidth,
      height: sliderHeight,
    });

    var slideCount = $(".carousel .slider ul li").length;
    var sliderUlWidth = slideCount * sliderWidth;

    $(".carousel .slider ul").css({
      width: sliderUlWidth,
  ...
  function moveLeft() {
    ...
    $(".carousel .slider ul").animate(
      {
        left: +slideWidth,
      },
      200,
      function () {
        $(".carousel .slider ul li:last-child").prependTo(
          ".carousel .slider ul"
        );
        $(".carousel .slider ul").css("left", "");
      }
    );
  }
  ...
  function moveRight() {
    ...
    $(".carousel .slider ul").animate(
      ...
      200,
      function () {
        $(".carousel .slider ul li:first-child").appendTo(
          ".carousel .slider ul"
        );
        $(".carousel .slider ul").css("left", "");
      }
